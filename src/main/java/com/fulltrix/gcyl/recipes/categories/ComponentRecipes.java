...
//TODO: nerf consumable macrowormhole generators
    ...
    private static void emitterInit() {
        ...
        for(int i = 9; i < 15; i++) {
            if (i == 14) {
                getAssLineResearchBuilder(i, 600, getEmitterByTier(i - 1).getStackForm(), false, false, V[MAX])
                ...
                getAssLineResearchBuilder(i, 600, getEmitterByTier(i - 1).getStackForm(), false, false, (int) (983040 * Math.pow(2, i - 9)))
                        ...
                        .input(circuit, getMarkerMaterialByTier(i), 2)
                        .input(foil, getSensorEmitterFoil(i), 64)
                        .input(foil, getSensorEmitterFoil(i), 64)
                        .input(cableGtDouble, getCableByTier(i), 8)
                        .fluidInputs(Indalloy140.getFluid((int) (L * 8 * Math.pow(2, i - 8))))
                        //.fluidInputs(getFluidMaterialByTier(i).getFluid(L * 4 * (i - 7)))
 ...
 */
     ...
     */
        ...
        for(int i = 9; i < 15; i++) {
            if (i == 14) {
                getAssLineResearchBuilder(i, 600, getSensorByTier(i - 1).getStackForm(), false, false, V[MAX])
    ...
    private static void fieldGenInit() {
        ...
        ASSEMBLY_LINE_RECIPES.recipeBuilder()
                ...
                .input(wireFine, UVSuperconductor, 64)
                .input(cableGtQuadruple, Duranium, 4)
        ...
        for(int i = 9; i < 15; i++) {
            if (i == 14) {
                ...
                getAssLineResearchBuilder(i, 600, getFieldGeneratorByTier(i - 1).getStackForm(), false, false, V[MAX])
                        ...
                        .input(wireFine, getSuperconductorByTier(i), 64)
                        .input(cableGtQuadruple, getCableByTier(i), 4)
 ...
 */
    ...
    private static void robotArmInit() {
        ...
        for(int i = 9; i < 15; i++) {
            if (i == 14) {
                getAssLineResearchBuilder(i, 600, getRobotArmByTier(i - 1).getStackForm(), false, false, V[MAX])
 ...
 */
        ...
        for(int i = 9; i < 15; i++) {
            if(i == 14) {
                getAssLineResearchBuilder(i, 600, getConveyorByTier(i - 1).getStackForm(), false, false, V[MAX])
...
/* original piston uv
        ...
        for(int i = 9; i < 15; i++) {
            if (i == 14) {
                getAssLineResearchBuilder(i, 600, getPistonByTier(i - 1).getStackForm(), false, false, V[MAX])
    ...
    private static void motorInit() {
        ...
        ASSEMBLY_LINE_RECIPES.recipeBuilder().duration(600).EUt(GTValues.V[GTValues.MAX])
    ...
    private static void pumpInit() {
        ...
        for(int i = 9; i < 15; i++) {
            if (i == 14) {
                getAssLineResearchBuilder(i, 600, getPumpByTier(i - 1).getStackForm(), false, false, V[MAX])
