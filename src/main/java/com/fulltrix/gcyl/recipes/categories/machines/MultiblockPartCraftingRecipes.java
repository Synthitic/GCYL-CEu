    ...
    private static void parallelHatches() {
        ...
        for (int i = 0; i < 5; i+=2) {
            getAssLineResearchBuilder(i+10, 1000, i == 0 ? PARALLEL_HATCH[3].getStackForm() : GCYL_PARALLEL_HATCH[i / 2 - 1].getStackForm(), false, false)
                    ...
                    .input(circuit, getMarkerMaterialByTier(i+11), (int) (16 * Math.pow(2,i)))
                    .inputs(getSensorByTier(i+10).getStackForm((int) (8 * Math.pow(2, i))))
                    .inputs(getEmitterByTier(i+10).getStackForm((int) (8 * Math.pow(2, i))))
