    ...
    public static void init() {
        ...
        for(int i = 0; i < 5; i++) {
            getAssLineResearchBuilder(i+9, 1000, i == 0 ? FUSION_REACTOR[2].getStackForm() : ADVANCED_FUSION_REACTOR[i-1].getStackForm(), false, false)
                    ...
                    .inputs(ULTRA_HIGH_POWER_INTEGRATED_CIRCUIT.getStackForm(i < 3 ? 32 : 64))
                    .inputs(ULTRA_HIGH_POWER_INTEGRATED_CIRCUIT.getStackForm(i < 3 ? 32 : 64))
                    .inputs(ULTRA_HIGH_POWER_INTEGRATED_CIRCUIT.getStackForm(i < 3 ? 32 : 64))
                    .inputs(ULTRA_HIGH_POWER_INTEGRATED_CIRCUIT.getStackForm(i < 3 ? 32 : 64))
                    .inputs(OreDictUnifier.get(wireGtDouble, getSuperconductorByTier(i+8), 64))
                    .inputs(OreDictUnifier.get(wireGtDouble, getSuperconductorByTier(i+8), 64))
                    .input(circuit, getMarkerMaterialByTier(i+9), 8)
                    .input(screw, i == 0 ? Trinium : getMainComponentMaterialByTier(i+8), 16)
                    .fluidInputs(Indalloy140.getFluid((int) (2880 * Math.pow(2, i))))
                    .buildAndRegister();
        }
