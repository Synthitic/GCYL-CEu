...
public class MixinWidgetOreList extends DraggableScrollableWidgetGroup {
    ...
    public MixinWidgetOreList(int xPosition, int yPosition, int width, int slotSize) {
    ...
    public void addOres(Set<String> ores, MixinProspectorMode mode) {
        switch (mode) {
            case ORE:
            ...
            case FLUID:
                ...
                break;
            case VIRTUAL_ORE:
                ores.stream().sorted().forEach(this::addVirtual);
            default:
                break;
        }
    }
    ...
    private void addVirtual(String orePrefix) {
        String[] oreList = Arrays.stream(orePrefix.split(",")).map(a -> ORE_PREFIX_CONSTANT_DISPLAY + Character.toTitleCase(a.charAt(0)) + a.substring(1)).collect(Collectors.joining(",")).split(",");

        for(int i = 0; i < oreList.length; i++) {
            if (ores.containsKey(oreList[i])) {
                return;
            }
            ItemStack itemStack = OreDictUnifier.get(oreList[i]);
            if (itemStack == null || itemStack.isEmpty()) return;
            ores.put(oreList[i], itemStack.getDisplayName());
            MaterialStack materialStack = OreDictUnifier.getMaterial(OreDictUnifier.get(oreList[i]));
            ItemStackHandler itemStackHandler = new ItemStackHandler(1);
            itemStackHandler.insertItem(0, itemStack, false);
            WidgetGroup widgetGroup = new WidgetGroup(0, 0, getSize().width - 5, 18);
            widgetGroup.addWidget(new SlotWidget(itemStackHandler, 0, 0, 0, false, false));
            widgetGroup.addWidget(new LabelWidget(20, 5, itemStack.getDisplayName(),
                    materialStack == null ? oreList[i].hashCode() : materialStack.material.getMaterialRGB() | 0XFF000000));
            addOrePrefix(oreList[i], widgetGroup);
        }
    }
