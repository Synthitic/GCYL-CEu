...
public class WirelessElectricStats implements IItemComponent, IItemCapabilityProvider, IItemMaxStackSizeProvider,
    ...
    public WirelessElectricStats(long maxCharge, long tier, boolean chargeable, boolean dischargeable) {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        WirelessElectricItem electricItem = (WirelessElectricItem) itemStack.getCapability(GregtechCapabilities.CAPABILITY_ELECTRIC_ITEM, null);

        if (electricItem != null && electricItem.canProvideChargeExternally() && player.isSneaking() && !electricItem.getInitialized()) {
            if (!world.isRemote) {
                boolean isInDischargeMode = isInDischargeMode(itemStack);
                setInDischargeMode(itemStack, !isInDischargeMode);
                electricItem.setPlayerUUID(player.getUniqueID());
                electricItem.setInitialized(true);
                electricItem.setPlayerName(player.getName());
                player.sendStatusMessage(new TextComponentTranslation("gcyl.wireless_initialized_private"), true);
            }
            return ActionResult.newResult(EnumActionResult.SUCCESS, itemStack);
        }

        if (electricItem != null && electricItem.canProvideChargeExternally() && !electricItem.getInitialized()) {
            if (!world.isRemote) {
                boolean isInDischargeMode = isInDischargeMode(itemStack);
                setInDischargeMode(itemStack, !isInDischargeMode);
                electricItem.setPlayerUUID(new UUID(0,0));
                electricItem.setInitialized(true);
                electricItem.setPlayerName("public");
                player.sendStatusMessage(new TextComponentTranslation("gcyl.wireless_initialized_public"), true);
            }
            return ActionResult.newResult(EnumActionResult.SUCCESS, itemStack);
        }
    ...
    public void onUpdate(ItemStack itemStack, Entity entity) {
        ...
        if (!entity.world.isRemote && entity instanceof EntityPlayer entityPlayer && electricItem != null &&  electricItem.canProvideChargeExternally() &&
                isInDischargeMode(itemStack) && electricItem.getCharge() != electricItem.getMaxCharge() && electricItem.getInitialized()) {
            VirtualEnergyRegistry.VirtualEnergyContainer virtualEnergyContainer = VirtualEnergyRegistry.getContainerCreate(makeEnergyContainerName(), electricItem.getPlayerUUID());
            if(virtualEnergyContainer.getEnergyStored() > electricItem.getTransferLimit()) {
                virtualEnergyContainer.remove(electricItem.charge(electricItem.getTransferLimit(), GTValues.MAX, true, false));
            }
        }
    ...
    public void addInformation(ItemStack itemStack, List<String> lines) {
        ...
        if (electricItem != null && electricItem.canProvideChargeExternally()) {
            ...
            if(!isInitialized(itemStack)) {
                lines.add(TextFormatting.RED + I18n.format("metaitem.wireless_electric.not_initialized.tooltip"));
            }
            else {
                if (isPrivate(itemStack)) {
                    lines.add(TextFormatting.LIGHT_PURPLE + I18n.format("metaitem.wireless_electric.private.tooltip", electricItem.getPlayerName()));
                }
                else {
                    lines.add(TextFormatting.LIGHT_PURPLE + I18n.format("metaitem.wireless_electric.public.tooltip"));
                }
            }
    ...
    private static boolean isPrivate(ItemStack itemStack) {
        NBTTagCompound tagCompound = itemStack.getTagCompound();
        if(tagCompound == null) {
            return false;
        }

        return !tagCompound.getString("playerName").equals("public");
    }

    private static boolean isInitialized(ItemStack itemStack) {
        NBTTagCompound tagCompound = itemStack.getTagCompound();
        if(tagCompound == null) {
            return false;
        }
    ...
    public int getMaxStackSize(ItemStack itemStack, int defaultValue) {
        WirelessElectricItem electricItem = (WirelessElectricItem) itemStack
    ...
    public ICapabilityProvider createProvider(ItemStack itemStack) {
        return new WirelessElectricItem(itemStack, maxCharge, tier, chargeable, dischargeable);
    }

    public static WirelessElectricStats createElectricItem(long maxCharge, long tier) {
        return new WirelessElectricStats(maxCharge, tier, true, false);
    }

    public static WirelessElectricStats createRechargeableBattery(long maxCharge, int tier) {
        return new WirelessElectricStats(maxCharge, tier, true, true);
    }

    public static WirelessElectricStats createBattery(long maxCharge, int tier, boolean rechargeable) {
        return new WirelessElectricStats(maxCharge, tier, rechargeable, true);
    }
